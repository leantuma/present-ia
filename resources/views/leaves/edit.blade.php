...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-900">{{ __('leaves.edit_title') }}</h1>
        <p class="mt-2 text-sm text-gray-600">{{ __('leaves.edit_description') }}</p>
    </div>
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select name="type" id="type" required
                        ...
                        <option value="vacation" {{ old('type', $leave->type) === 'vacation' ? 'selected' : '' }}>{{ __('leaves.vacation') }}</option>
                        <option value="sick" {{ old('type', $leave->type) === 'sick' ? 'selected' : '' }}>{{ __('leaves.sick') }}</option>
                        <option value="maternity" {{ old('type', $leave->type) === 'maternity' ? 'selected' : '' }}>{{ __('leaves.maternity') }}</option>
                        <option value="other" {{ old('type', $leave->type) === 'other' ? 'selected' : '' }}>{{ __('leaves.other') }}</option>
                    </select>
                    @error('type')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" id="days_count" readonly
                        ...
                        value="{{ $leave->getDaysCount() }} días">
                </div>

                <div>
                    <label for="reason" class="block text-sm font-medium text-gray-700">{{ __('leaves.reason') }}</label>
                    <textarea name="reason" id="reason" rows="4"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary @error('reason') border-red-300 @enderror">{{ old('reason', $leave->reason) }}</textarea>
                    @error('reason')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
