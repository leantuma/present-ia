...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-900">{{ __('companies.create_title') }}</h1>
        <p class="mt-2 text-sm text-gray-600">{{ __('companies.create_description') }}</p>
    </div>
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="address" id="address" rows="3"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary @error('address') border-red-300 @enderror">{{ old('address') }}</textarea>
                    @error('address')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="language" id="language" required
                        ...
                        <option value="es" {{ old('language', 'es') === 'es' ? 'selected' : '' }}>{{ __('companies.spanish') }}</option>
                        <option value="en" {{ old('language') === 'en' ? 'selected' : '' }}>{{ __('companies.english') }}</option>
                    </select>
