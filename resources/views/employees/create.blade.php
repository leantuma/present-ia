...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-900">{{ __('employees.create_title') }}</h1>
        <p class="mt-2 text-sm text-gray-600">{{ __('employees.create_description') }}</p>
    </div>
    ...
    <div class="bg-white shadow-sm rounded-lg p-6">
        <form action="{{ route('employees.store') }}" method="POST">
            ...
            <div class="space-y-6">
                ...
                <div>
                    <label for="password" class="block text-sm font-medium text-gray-700">{{ __('employees.password') }} <span class="text-red-500">*</span></label>
                    ...
                    <p class="mt-1 text-sm text-gray-500">{{ __('employees.password_min') }}</p>
                    @error('password')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="employee_id" id="employee_id" value="{{ old('employee_id') }}"
                        ...
                        placeholder="{{ __('employees.employee_id_placeholder') }}">
                    @error('employee_id')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
